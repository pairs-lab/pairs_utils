...
#include <pairs_circle_flier/pairs_circle_flierConfig.h>

//}

namespace pairs_circle_flier
{
...
private:
  ...
  bool setTrajectorySrv(const pairs_msgs::TrajectoryReference trajectory);
  ...
  typedef pairs_circle_flier::pairs_circle_flierConfig      DrsParams_t;
  typedef dynamic_reconfigure::Server<DrsParams_t> Drs_t;
  boost::shared_ptr<Drs_t>                         drs_;
  void                                             callbackDrs(pairs_circle_flier::pairs_circle_flierConfig& params, uint32_t level);
  DrsParams_t                                      params_;
...
void CircleFlier::onInit(void) {
  ...
  pairs_lib::ParamLoader param_loader(nh_, "CircleFlier");
  ...
  service_client_trajectory_ = nh_.serviceClient<pairs_msgs::TrajectoryReferenceSrv>("trajectory_reference_out");
  publisher_trajectory_      = nh_.advertise<pairs_msgs::TrajectoryReference>("trajectory_reference_out", 1, false);
...
void CircleFlier::callbackDrs(pairs_circle_flier::pairs_circle_flierConfig& params, [[maybe_unused]] uint32_t level) {
...
void CircleFlier::timerMain([[maybe_unused]] const ros::TimerEvent& event) {
  ...
  auto angle     = pairs_lib::get_mutexed(mutex_global_, angle_);
  auto cycloid_x = pairs_lib::get_mutexed(mutex_global_, cycloid_x_);
  auto heading   = pairs_lib::get_mutexed(mutex_global_, heading_);
  ...
  angle        = pairs_lib::get_mutexed(mutex_global_, angle_);
  angle += direction * angular_step;
  angle = radians::wrap(angle);
  pairs_lib::set_mutexed(mutex_global_, angle, angle_);

  // do a step with the global heading_
  if (params.heading_mode == 3) {
    heading = pairs_lib::get_mutexed(mutex_global_, heading_);
    heading += params.heading_rate * (1.0 / params.publisher_rate);
    heading = radians::wrap(heading);
    pairs_lib::set_mutexed(mutex_global_, heading, heading_);
  }

  // do a step with the cycloid
  if (params.orientation == 3) {
    cycloid_x = pairs_lib::get_mutexed(mutex_global_, cycloid_x_);
    cycloid_x += params.cycloid_speed * (1.0 / params.publisher_rate);
    pairs_lib::set_mutexed(mutex_global_, cycloid_x, cycloid_x_);
  }
...
bool CircleFlier::setTrajectorySrv(const pairs_msgs::TrajectoryReference trajectory) {
...
PLUGINLIB_EXPORT_CLASS(pairs_circle_flier::CircleFlier, nodelet::Nodelet)
