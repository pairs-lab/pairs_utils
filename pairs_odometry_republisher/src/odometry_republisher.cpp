...
class OdometryRepublisher : public pairs_lib::Node {
public:
  // virtual void onInit();
...
OdometryRepublisher::OdometryRepublisher(const rclcpp::NodeOptions &options) : pairs_lib::Node("OdometryRepublisher", options) {
  initialize();
}
...
void OdometryRepublisher::initialize() {
  ...
  sh_odometry_ = pairs_lib::SubscriberHandler<nav_msgs::msg::Odometry>(shopts, "~/odometry_in", &OdometryRepublisher::callbackOdometry, this);
  ...
  ph_odometry_ = pairs_lib::PublisherHandler<nav_msgs::msg::Odometry>(phopts, "~/odometry_out");
...
void OdometryRepublisher::callbackOdometry(const nav_msgs::msg::Odometry::ConstSharedPtr msg) {
  ...
  if (_should_transform_) {
    ...
    if (_should_transform_from_ros_) {
      ...
      R_IMU_FCU = pairs_lib::AttitudeConverter(tf_from_ros.transform.rotation);

    } else {

      R_IMU_FCU = pairs_lib::AttitudeConverter(_transform_R_);
    }
  ...
  Eigen::Matrix3d R_GLOBAL_IMU = pairs_lib::AttitudeConverter(msg->pose.pose.orientation);

  // R^GLOBAL_FCU = R^GLOBAL_IMU * R^IMU_FCU
  pose_transformed.orientation = pairs_lib::AttitudeConverter(R_GLOBAL_IMU * R_IMU_FCU);
  ...
  if (_init_heading_to_zero_) {
    ...
    if (!got_init_pose_) {
      init_hdg_ = pairs_lib::AttitudeConverter(pose_transformed.orientation).getHeading();
    ...
    tf_msg.transform.rotation      = pairs_lib::AttitudeConverter(0, 0, 0).setHeading(-init_hdg_);
    ...
    // obtain transformed covariance as R^PAIRS_GLOBAL * covariance * (R^PAIRS_GLOBAL)^T where covariance is each 3x3 block of the 6x6 covariance matrix
  ...
  // publish the initial offset to TF - T^GLOBAL_PAIRS
  ...
  tf_msg_inv.transform.rotation      = pairs_lib::AttitudeConverter(0, 0, 0).setHeading(init_hdg_);
  ...
  if (_velocity_in_body_frame_) {
    ...
    // R^GLOBAL_FCU = R^GLOBAL_IMU * R^IMU_FCU
    Eigen::Matrix3d R_GLOBAL_FCU = pairs_lib::AttitudeConverter(R_GLOBAL_IMU * R_IMU_FCU);
    Eigen::Vector3d v2;
    v2 << linear_velocity.x, linear_velocity.y, linear_velocity.z;
  ...
  try {
    heading_out = pairs_lib::AttitudeConverter(odom_transformed.pose.pose.orientation).getHeading();
  }
  catch (...) {
