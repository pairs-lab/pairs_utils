...
void TfReconfigure::onInit() {
  ...
  reconfigure_server_.reset(new dynamic_reconfigure::Server<pairs_tf_reconfigure::tfConfig>(mutex_reconfigure_, nh_));
  dynamic_reconfigure::Server<pairs_tf_reconfigure::tfConfig>::CallbackType f = boost::bind(&TfReconfigure::callbackReconfigure, this, _1, _2);
  reconfigure_server_->setCallback(f);

  transformer_ = std::make_unique<pairs_lib::Transformer>("pairs_tf_reconfigure");
...
/* //{ callbackReconfigure() */
void TfReconfigure::callbackReconfigure([[maybe_unused]] pairs_tf_reconfigure::tfConfig& config, [[maybe_unused]] uint32_t level) {
...
PLUGINLIB_EXPORT_CLASS(pairs_tf_reconfigure::TfReconfigure, nodelet::Nodelet)
