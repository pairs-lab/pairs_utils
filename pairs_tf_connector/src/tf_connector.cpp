...
class TFConnector : public pairs_lib::Node {
public:
  /* TFConnector //{ */
  TFConnector(const rclcpp::NodeOptions& options) : pairs_lib::Node("TFConnector", options) {
    node_ = this_node_ptr();
    ...
    m_pub_tf  = pairs_lib::PublisherHandler<tf2_msgs::msg::TFMessage>(phopts, "~/tf_out");

    m_ddynrec = std::make_shared<pairs_lib::DynparamMgr>(node_, mutex_drs_params_);
    ...
    m_sub_tf          = pairs_lib::SubscriberHandler<tf2_msgs::msg::TFMessage>(shopts, "~/tf_in", &TFConnector::tf_callback, this);

    //}

    if (m_max_update_period > 0) {
      pairs_lib::TimerHandlerOptions thopts;
      thopts.node      = node_;
...
private:
  ...
  pairs_lib::SubscriberHandler<tf2_msgs::msg::TFMessage> m_sub_tf;
  pairs_lib::PublisherHandler<tf2_msgs::msg::TFMessage>  m_pub_tf;
  std::shared_ptr<TimerType>                           m_tim_tf;
  std::shared_ptr<pairs_lib::DynparamMgr>                m_ddynrec;
...
public:
  ...
  void initialize_ddynrec() {
    for (auto& el : m_frame_connections) {
      ...
      m_ddynrec->register_param(group_in + "/x", &(el->override_in_x), 0.0, pairs_lib::DynparamMgr::range_t<double>(-100.0, 100.0));
      m_ddynrec->register_param(group_in + "/y", &(el->override_in_y), 0.0, pairs_lib::DynparamMgr::range_t<double>(-100.0, 100.0));
      m_ddynrec->register_param(group_in + "/z", &(el->override_in_z), 0.0, pairs_lib::DynparamMgr::range_t<double>(-100.0, 100.0));
      m_ddynrec->register_param(group_in + "/heading", &(el->override_in_heading), 0.0, pairs_lib::DynparamMgr::range_t<double>(-M_PI, M_PI));

      const std::string group_ex = frame_name + "/extrinsic";
      m_ddynrec->register_param(group_ex + "/override", &(el->override_ex), false);
      m_ddynrec->register_param(group_ex + "/x", &(el->override_ex_x), 0.0, pairs_lib::DynparamMgr::range_t<double>(-100.0, 100.0));
      m_ddynrec->register_param(group_ex + "/y", &(el->override_ex_y), 0.0, pairs_lib::DynparamMgr::range_t<double>(-100.0, 100.0));
      m_ddynrec->register_param(group_ex + "/z", &(el->override_ex_z), 0.0, pairs_lib::DynparamMgr::range_t<double>(-100.0, 100.0));
      m_ddynrec->register_param(group_ex + "/heading", &(el->override_ex_heading), 0.0, pairs_lib::DynparamMgr::range_t<double>(-M_PI, M_PI));
    }
  }
  //}
};
...
RCLCPP_COMPONENTS_REGISTER_NODE(pairs_tf_connector::TFConnector)
