...
class TfMirror : public pairs_lib::Node {
...
private:
  ...
  std::vector<pairs_lib::SubscriberHandler<nav_msgs::msg::Odometry>> sh_odoms_;
...
TfMirror::TfMirror(rclcpp::NodeOptions options) : Node("TfMirror", options) {
  ...
  for (auto &tf : _tfs_) {
    ...
    if (params.odom_source_enabled) {
      sh_odoms_.push_back(pairs_lib::SubscriberHandler<nav_msgs::msg::Odometry>(shopts, params.odom_topic,
...
RCLCPP_COMPONENTS_REGISTER_NODE(pairs_tf_mirror::TfMirror)
