...
private:
  ...
  std::vector<pairs_lib::SubscribeHandler<nav_msgs::Odometry>> sh_odoms_;
  ...
  std::shared_ptr<pairs_lib::TransformBroadcaster> tf_dynamic_broadcaster_;
...
void TfMirror::onInit() {
  ...
  tf_dynamic_broadcaster_ = std::make_shared<pairs_lib::TransformBroadcaster>();

  // | --------------------- tf transformer --------------------- |

  transformer_ = std::make_shared<pairs_lib::Transformer>(nh_, "TfMirror");
  ...
  pairs_lib::ParamLoader param_loader(nh_, "TfMirror");
  ...
  for (auto& tf : _tfs_) {
    ...
    if (params.odom_source_enabled) {
      sh_odoms_.push_back(pairs_lib::SubscribeHandler<nav_msgs::Odometry>(shopts, params.odom_topic,
...
PLUGINLIB_EXPORT_CLASS(pairs_tf_mirror::TfMirror, nodelet::Nodelet)
